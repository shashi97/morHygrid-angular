import {
    ...
    OnDestroy, OnInit, Input, Output, SimpleChange, EventEmitter, forwardRef, Renderer2,
    ...
    template: `
        <span [ngClass]="{'ui-calendar':true,'ui-calendar-w-btn':showIcon}" [ngStyle]="style" [class]="styleClass">
            <ng-template [ngIf]="!inline">
                <input #inputfield type="text" [attr.id]="inputId" [attr.name]="name" [attr.required]="required" [value]="inputFieldValue" (focus)="onInputFocus($event)" (keydown)="onInputKeydown($event)" (change)="onInputChange($event)" (click)="datepickerClick=true" (blur)="onInputBlur($event)"
                        ...
                        <tr *ngFor="let week of dates">
                            <td *ngFor="let date of week" [ngClass]="{'ui-datepicker-other-month ui-state-disabled':date.otherMonth,
                                ...
                                <a class="ui-state-default" href="#" *ngIf="date.otherMonth ? showOtherMonths : true" 
                                    ...
                                    (click)="onDateSelect($event,date,false)">
...
export class Calendar implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy, ControlValueAccessor {
    ...
    onDateSelect(event, dateMeta, call) {
        if (!call) {
    ...
    isDateEquals(value, dateMeta) {
        if (value) {
            if (value !== '$Today' &&
                ...
                value !== '$EndOfPrevMonth' &&
                value !== '$EndOfPrevQuarter' &&
                value !== '$EndOfLastYear' &&
                value !== '$StartOfThisMonth' &&
                value !== '$StartOfThisQuarter' &&
                value !== '$StartOfThisYear') {
                return value.getDate() === dateMeta.day && value.getMonth() === dateMeta.month && value.getFullYear() === dateMeta.year;
            } else {
                return new Date().getDate() === dateMeta.day
                    && new Date().getMonth() === dateMeta.month
                    && new Date().getFullYear() === dateMeta.year;
            }
    ...
    updateUI() {
        ...
        if (val === '$Today' ||
            ...
            val === '$PrevBusinessDay' ||
            val === '$EndOfPrevMonth' ||
            val === '$EndOfPrevQuarter' ||
            val === '$EndOfLastYear' ||
            val === '$StartOfThisMonth' ||
            val === '$StartOfThisQuarter' ||
            val === '$StartOfThisYear') {
                return false;
        }
    ...
    onTodayButtonClick(event) {
        ...
        this.onDateSelect(event, dateMeta, 'false');
